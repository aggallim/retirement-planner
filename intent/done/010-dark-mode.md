...
Raised via the user's own roadmap tracking (the project's Notion page)
rather than `docs/TOOL_DOCUMENTATION.md` §7 itself, which never actually
named dark mode as a line item even before its "Possible future
additions" checklist was removed entirely by
`intent/done/009-tool-docs-history-cleanup.md` (that checklist is now
Notion-only, per that requirement).

(Numbered 010, not 006 or 008, after two renumberings while this was in
progress: 006 and 007 were claimed by the mobile Data-menu overflow fix
and the dev-workflow-improvements requirement
(`intent/done/006-mobile-data-menu-overflow.md`,
`intent/done/007-dev-workflow-improvements.md`); 008 was then claimed by
`intent/008-dismissable-warning-banner.md`, drafted but not yet
implemented, while this branch was mid-flight a second time.)
