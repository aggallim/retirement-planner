# 018 — Relicense from GPLv3 to all-rights-reserved

## Status

Resolved via the `grilling` skill, run in two rounds. Standalone item — not
bundled with roadmap item #12 (Simple/Advanced mode split), which depends on
this one but is separate functional work with its own intent later.
...
- **No external contributors or forks exist.** `git log` shows only Aaron
  ...
  shows 0 forks and 0 stars on the repo (created 2026-09-15, so 8 days old
  at the time of this intent). As sole copyright holder, Aaron has full
  legal authority to relicense without anyone else's consent — confirmed
  against GitHub's own guidance and general GPL-relicensing practice (no
  multi-contributor complication applies here).
...
3. **No retroactive-GPLv3 caveat.** GPL grants are technically irrevocable
   ...
   evidence of any external clone in the repo's 8-day public history, an
...
4. **README gets an explicit license note.** `README.md` currently says
   ...
   it's no longer open source, a short explicit note (or a new `## License`
...
- The exact LICENSE wording and README placement are spec-level detail,
  ...
  repository owner / `aggallim`) and a matching short README note or
  `## License` section, consistent with decisions 1, 2 and 4 above.
